...
class Transmitter():
    ...
    def packets_to_queue(self, file_path, callsign, Qout, bytes_read=256):
        ...
        while(1):
            ...
            tmp = self.tnc.modulatePacket(callsign, "", str(npp), bytes, preflags=40, postflags=40 )
            Qout.put(tmp)
            npp = npp+1
    ...
    def generate_packets(self, Qout, callsign, file_path):
        ...
        Qout.put("KEYON")
        tmp = self.tnc.modulatePacket(callsign, "", "BEGIN", file_path , preflags=40, postflags=40 )
        Qout.put(tmp)
